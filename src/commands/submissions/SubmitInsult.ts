...
function main(message: Message, formattedMessage: FormattedMessage, middleware: CentralizedMiddleware): void {
    ...
    if (isBigMan) {
        ...
        message.reply("your insult was submitted. bigman council will review it and accept/decline it").then(async (sentMessage) => {  //Sends the message that the submission has been received
            ...
            let submissionMessage = await (<TextChannel>message.guild.channels.resolve(middleware.guildMiddleware.insultSubmission)).send({embeds: [embed]}) //Sends the message to submissions channel
